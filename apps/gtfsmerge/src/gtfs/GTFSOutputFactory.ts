...
export class GTFSOutputFactory {
   ...
   * not compile.
   */
  public create(extra: ExtraColumns["write"] = {}): GTFSOutput {
    ...
    const calendar = new DedupingWriter(open(CALENDAR), row => String(row.service_id));
    const routes = new DedupingWriter(open(ROUTES), row => String(row.route_id));
    const agency = new DedupingWriter(open(AGENCY), row => String(row.agency_id));
    const stops = new DedupingWriter(open(STOPS), row => row.stop_id);
    const areas = new DedupingWriter(open(AREAS), row => String(row.area_id));
    const stopAreas = new DedupingWriter(open(STOP_AREAS), row => `${row.area_id}_${row.stop_id}`);
    ...
    const calendarDates = open(CALENDAR_DATES);
    const trips = open(tripsFile(this.shapes));
    const stopTimes = open(STOP_TIMES);
    const transfers = open(TRANSFERS);
    const frequencies = open(FREQUENCIES);
    // Not opened at all when the merge carries no shapes, so the feed has no
    // empty shapes.txt to explain.
    const shapes = this.shapes ? open(SHAPES) : undefined;
    ...
    return new GTFSOutput(
      ...
      new FeedInfoMerger(open(FEED_INFO)),
