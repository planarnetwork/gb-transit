...
- **Accessibility and station pages come from the National Rail Knowledgebase.**
  ...
  platform has one; `stop_url` is the station's page on nationalrail.co.uk.
...
- **Accessibility is station-level only.** `wheelchair_boarding` is the step-free access category
  the National Rail Knowledgebase publishes for the station as a whole. It is not per platform, so
  a station with step-free access to two of its four platforms is `1` like one with access to all
  four, and it is `0` — no information — for the 37 stations the Knowledgebase has not classified.
  The station's own page, in `stop_url`, has the detail.
