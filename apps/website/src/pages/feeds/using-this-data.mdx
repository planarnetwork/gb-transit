...
A journey planner wants the first, and so does a map: every trip is drawn as a [shape](#shapes)
through the stations it runs through as well as the ones it calls at.

A feed being merged with the tube and bus timetables wants the second, which has fewer trips than the
first. See [the National Rail only feed](#the-national-rail-only-feed).

A journey planner for journeys that cross London wants the third: the second with TfL's own
timetables for the tube, DLR, trams, river boats and cable car beside it. See
[the rail and TfL feed](#the-rail-and-tfl-feed).
...
| asset | what it is |
|---|---|
| `validation.json` | the validator's report, including every accepted error. One per feed, the others suffixed as their zips are |
...
Five things are worth knowing before joining anything to `stops.txt`:
...
- **A station with no coordinate is either absent or at 0,0.** One that nothing references is not
  ...
  position.
- **Accessibility and station pages come from the National Rail Knowledgebase.**
  `wheelchair_boarding` is `1` wherever a step-free path to a platform exists and `2` only where no
  platform has one. It is set on the station; a boarding point is `0`, which the spec reads as
  "inherit from the station", because no source says which of a station's platforms are reachable.
  `stop_url` is the station's page on nationalrail.co.uk.
- **Coordinates and station names come from NaPTAN**, which supersedes the upper case, sixteen
  character names the timetable carries. Boarding points are created after enrichment, as copies of
  the station, so a platform carries the same surveyed coordinate and the same readable name — a
  platform of `Newcastle Airport Metro`, never of `NEWCASTLE AIRPRT`. `wheelchair_boarding` is the
  exception, and deliberately: see above.
...
Every trip carries a `shape_id`, and `shapes.txt` is the line it runs over: a polyline through the
stations the train touches, whether it calls there or runs through. The places it runs through are
not in `stop_times.txt`, but the line is drawn through them, and the nightly stops rather than
publishing a feed whose lines have lost them.

The passing points are the whole reason the line is worth drawing. Without them a King's Cross to
Newcastle service is a single 395 km straight line; with them it is twenty-odd hops that trace the
East Coast Main Line. Across the feed they cut the number of straight-line hops longer than 50 km
from 4,751 to 404.

**It is not the track.** 2,170,472 of the CIF's 3,062,488 passing points are junctions, loops and
signal boxes — `WATRLWC`, `LEEDSWJ`, `HTRWAJN` — and the DTD gives no coordinate for any of them.
What is left is a station-to-station sketch: enough to tell a Bristol train from a Birmingham one on
a map, not enough to draw the railway. The median hop is a 4.8 km straight line over ground the
rails curve across.

Two consequences worth knowing:

- **A shape is shared.** One line on the ground carries every stopping pattern that runs over it, so
  the fast and the stopper have the same `shape_id` — 13,722 shapes for 278,794 trips. The id is a
  digest of the stations the line runs through, so it is the same id in every build and can be
  referred to from outside the feed.
- **There is no `shape_dist_traveled`.** GTFS only reads one where `stop_times.txt` carries it too,
  and putting a distance on all 2.9 million calls to disambiguate the 2.5% of trips that pass one
  station twice is not worth the largest file in the feed growing for it.

A trip whose every station the feed cannot place has an empty `shape_id` rather than a line drawn
through Null Island.
...
## The rail and TfL feed

`gtfs-rail-and-tfl.zip` is the National Rail only feed with TfL's own timetables for the tube, the
DLR, the trams, the river boats and the cable car beside it, from TfL's
[Journey Planner Timetables](https://tfl.gov.uk/info-for/open-data-users/our-open-data). It is for
planning a journey across London on the trains that actually run: the other two feeds cross London
on the DTD's fixed links, which say the tube takes a fixed number of minutes whatever the time of
day.

The rail half is the National Rail only feed without those tube links. Its walks, and its links
of every other mode, stay. The TfL half covers the same days. TfL publishes an engineering works
timetable as a service of its own on top of the one it interrupts, and on the days it applies it
replaces that service rather than running beside it.

**A TfL station inside a rail station is one of its platforms.** Liverpool Street's tube platforms
are children of `910GLIVST`, so the rail station's minimum connection time is the interchange for
every change there — train to tube, tube to train, and tube to tube. The rail feed charges a
station's connection time on every arrival, so a tube station of its own, with a walk to it, would
charge both stations' times on every change between the two. A tube station that is not inside a
rail station is a station of its own, with a walk to the rail stations and other TfL stations near
it, timed by distance.

TfL's ids are its own. A stop is its NaPTAN ATCO code, as the rail feed's are — `9400ZZLUBST1` is a
Baker Street platform and `940GZZLUBST` the station — and TfL's trips, routes and services are
prefixed `tfl_`. A TfL station of its own carries a three character `stop_code` starting with a
digit, which no CRS code does, for the [transfer patterns](#the-transfer-patterns) planner. The
codes are numbered in the order of the stations' ids, so they are stable within a release and not
between them.

A TfL line is one route, `tfl_BAK` for the Bakerloo, whichever of TfL's timetables a trip came
from, in the line's colour from TfL's
[Colour Standard](https://content.tfl.gov.uk/tfl-colour-standard.pdf) and linking to its page on
tfl.gov.uk. TfL's trips are drawn station to station, like the rail feed's: the timetables say which
stops a train calls at and nothing of the line between them, so the shape is a straight line from each
stop to the next.

TfL's open data licence makes the acknowledgement "Powered by TfL Open Data" a condition of using
the timetables, and the feed's `attributions.txt` carries it.
...
stations it takes from the line above and what follows it, and the whole thing is gzip compressed —
which is what a browser can decompress, where brotli is not. It needs no marker for a pattern that a longer one runs through, because every line is
exactly one pattern.
...
- **Accessibility is station-level only.** `wheelchair_boarding` is the step-free access category
  the National Rail Knowledgebase publishes for the station as a whole, so a station with step-free
  access to two of its four platforms is `1` like one with access to all four. Boarding points
  carry `0` rather than repeating their station's value, because nothing says which platforms those
  two are. It is `0` on the station too for the stations the Knowledgebase has not classified —
  37 of the 2,613 it lists. The station's own page, in `stop_url`, has the detail.
