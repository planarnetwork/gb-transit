...
export async function loadFares(source: string | string[], options: LoadFaresOptions = {}): Promise<FaresData> {
  return new FaresLoader(new FaresFeed(source), options.date).load();
}

/**
 * Reads each file of a fares feed into the records valid on a date, interning the codes of flows and fares as it goes
 */
class FaresLoader {

  private readonly date: DateNumber | undefined;
  ...
  constructor(private readonly feed: FaresFeed, date?: string) {
    this.date = date === undefined ? undefined : dateNumber(date);
    this.codes.restrictions.id("");
  }

  public async load(): Promise<FaresData> {
    const [locations, clusters, ticketTypes, nonDerivableFares, flowsAndFares] = await Promise.all([
      this.readLocations(),
      this.readClusters(),
      this.readTicketTypes(),
      this.readNonDerivableFares(),
      this.readFlows()
    ]);
  ...
  private valid(start: DateNumber, end: DateNumber): boolean {
    return this.date === undefined || (start <= this.date && this.date <= end);
  }

  private async readLocations() {
    const locations: Location[] = [];
    const groups = new Map<string, LocationGroup>();
    const members: [key: string, member: LocationGroupMember][] = [];

    await this.feed.eachLine("LOC", LOC, line => {
      ...
      if (type === "L") {
        ...
        if (this.valid(startDate, endDate)) {
          locations.push({
            uic: text(v.uic)!,
            nlc: text(v.nlc),
            crs: text(v.crs),
  ...
  private async readClusters(): Promise<StationCluster[]> {
    const clusters: StationCluster[] = [];

    await this.feed.eachLine("FSC", FSC, line => {
  ...
  private async readTicketTypes(): Promise<TicketType[]> {
    const ticketTypes: TicketType[] = [];

    await this.feed.eachLine("TTY", TTY, line => {
  ...
  private async readNonDerivableFares(): Promise<NonDerivableFare[]> {
    const fares: NonDerivableFare[] = [];

    await this.feed.eachLine("NFO", NFO, line => {
   ...
   */
  private async readFlows(): Promise<{flows: Flows, fares: Fares}> {
    ...
    await this.feed.eachLine("FFL", FFL, line => {
      ...
      if (type === 84) {
        ...
        fareColumns.ticket.push(this.codes.tickets.id(line.substr(fare.ticket_code[0], fare.ticket_code[1])));
        fareColumns.price.push(parseInt(line.substr(fare.fare[0], fare.fare[1]), 10));
        fareColumns.restriction.push(isBlank(restriction) ? 0 : this.codes.restrictions.id(restriction));
      }
      else if (type === 70) {
        ...
        if (this.valid(startDate, endDate)) {
          ...
          flowColumns.origin.push(this.codes.locations.id(line.substr(flow.origin_code[0], flow.origin_code[1])));
          flowColumns.destination.push(this.codes.locations.id(line.substr(flow.destination_code[0], flow.destination_code[1])));
          flowColumns.route.push(this.codes.routes.id(line.substr(flow.route_code[0], flow.route_code[1])));
