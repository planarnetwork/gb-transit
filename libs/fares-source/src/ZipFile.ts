 ...
 *
 * The other zip readers here do not fit. @gb-transit/gtfs-loader's readZip streams a zip from the front with fflate,
 * so reaching an entry means inflating every entry before it, in JavaScript. @gb-transit/dtd-source's FeedZip reads the
 * whole zip with adm-zip and extracts each entry to a temporary file. A fares feed is read one file at a time, in
 * whatever order the change files need, and the flows are 200MB uncompressed.
 *
...
export class ZipFile {
  ...
  private constructor(
    ...
    public readonly entries: readonly ZipFileEntry[]
  ) {}
  ...
  public static open(path: string): ZipFile {
    ...
    try {
      return new ZipFile(path, ZipFile.readCentralDirectory(fd, path));
    }
    finally {
      fs.closeSync(fd);
    }
  ...
  /**
   * The decompressed content of an entry. The file is closed when the stream ends, fails or is destroyed.
   */
  public stream(entry: ZipFileEntry): Readable {
    if (entry.compressedSize === 0) {
      return Readable.from([]);
    }

    const fd = fs.openSync(this.path, "r");
    let raw: fs.ReadStream;
    ...
    try {
      ...
      raw = fs.createReadStream("", {fd, start, end: start + entry.compressedSize - 1, highWaterMark: 1 << 20});
    }
    catch (err) {
      fs.closeSync(fd);
      throw err;
    ...
    // pipeline destroys each stream when the other fails or is destroyed, which a plain pipe does not
    return pipeline(raw, zlib.createInflateRaw({chunkSize: 1 << 20}), () => {});
  }

  /**
   * Call back with every line of an entry, without line endings. Blank lines and the `/!!` comments are skipped.
   */
  public async eachLine(entry: ZipFileEntry, onLine: (line: string) => void): Promise<void> {
    let remainder = "";

    for await (const chunk of this.stream(entry)) {
  ...
  private static readCentralDirectory(fd: number, path: string): ZipFileEntry[] {
